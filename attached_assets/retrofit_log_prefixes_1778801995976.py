...
# --- Regexes (shared with repair_flags_from_log.py) ---
...
def _maybe_extract_event_marker(body):
    ...
    """
    # NEW format: ✦ [tier] 'Name' | date
    m = RE_NEW_EVENT.search(body)
    if m:
        date = m.group(3).strip()
        return (m.group(2).strip(), date)
    # OLD format: ✓ EVENT FOUND: Name (date on a later line, can't disambiguate yet)
...
def retrofit_one_log(in_path, out_path, mode, sheet_index):
    ...
    # Tracking state
    candidate = None         # most-recent Processing post: ID (legacy walker)
    last_event_post = None   # post_id we last assigned an event marker to
                             # (so trailing field/Flags bullets attach there)
    ...
    for raw_line in text.splitlines():
        ...
        if m_pre:
            ...
            # Still track candidate for unprefixed lines later in the file
        ...
        # Update candidate when we see a Processing post: marker
        ...
        # Decide attribution for this line.
        # Default: most-recent candidate.
        # Smart mode: if line is an event marker and sheet says a different
        # post owns this (name, date), reassign and remember for trailing bullets.
        assigned = candidate
        was_corrected = False
        ...
        if mode == 'smart' and sheet_index:
            ...
            if ev is not None:
                ...
                # Try without date if no hit (OLD-format event lines have no date)
                if not pids and not ev_date:
                    # Search by name only
                    matches = [pid for (n, d), pids2 in sheet_index.items()
                               for pid in pids2 if n == _normalize_name(ev_name)]
                    pids = list(dict.fromkeys(matches))  # unique, preserve order
                if pids:
                    # If candidate already matches, keep candidate.
                    if candidate in pids:
                        ...
                        # Ambiguous (event name+date appears under multiple posts);
                        # don't override candidate.
                        assigned = candidate
                last_event_post = assigned

        # For non-event lines in smart mode, also use last_event_post if it
        # diverged from candidate (so '• Flags:' bullets follow the event).
